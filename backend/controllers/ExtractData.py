...
async def extractData(pdf_file, pages):
    try:
        ...
        for k, v in pages_dict.pages.items():
            start_idx = v[0]
            end_idx = v[1]
            pages_to_extract = []
            ...
            if not os.path.exists(config.Dirs.PROGRESS_DIR + "/" + str(k) + "_" + pdf_file.filename + "/" + config.Dirs.PROGRESS_FILE):
                os.makedirs(config.Dirs.PROGRESS_DIR + "/" + str(k) + "_" + pdf_file.filename, exist_ok=True)
            
            processed_pages = extractDataAndSave.load_progress(
                progress_path=config.Dirs.PROGRESS_DIR + "/" + str(k) + "_" + pdf_file.filename + "/" + config.Dirs.PROGRESS_FILE
            )
            
            pages_remaining = list(set(pages_to_extract) - set(processed_pages))
        
            if pages_remaining:
                extracted_pages_map = extractDataAndSave.extract_pages(config.Dirs.PDF_DIR + "/" + pdf_file.filename, pages_remaining)
                for page_num, text in extracted_pages_map.items():
                    logger.info(f"Starting for page {page_num} for {pdf_file.filename}")
                    ...
                    if not structured_text:
                        ...
                        if not os.path.exists(config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename):
                            os.makedirs(config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename, exist_ok=True)
                            
                        extractDataAndSave.save_text_file(
                            structured_text, 
                            page_num, 
                            output_dir=config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename
                        )
                        
                        processed_pages.add(page_num)
                        
                        extractDataAndSave.save_progress(
                            processed_pages, progress_path=config.Dirs.PROGRESS_DIR + "/" + str(k) + "_" + pdf_file.filename + "/" + config.Dirs.PROGRESS_FILE
                        )
            ...
            for page_num in pages_to_extract:
                txt_path = os.path.join(config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename, f"page_{page_num}_extracted.txt")
                json_path = os.path.join(config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename, f"page_{page_num}_data.json")
                ...
                if not os.path.exists(txt_path):
                    ...
                    extracted_map = extractDataAndSave.extract_pages(pdf_file, [page_num])
                ...
                if json_result:
                    saved = extractDataAndSave.save_json_file(json_result, page_num, output_dir=config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename)
                    if saved:
            ...
            if successful_json_pages > 0:
                ...
                combined_json = extractDataAndSave.combine_with_ollama_structured(config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename, pages=pages_to_extract, batch_size=3)

                if not combined_json:
                    logger.warning("Falling back to non-structured Ollama combine...")
                    combined_json = extractDataAndSave.combine_with_ollama_from_json(config.Dirs.JSON_DIR + "/" + str(k) + "_" + pdf_file.filename, pages=pages_to_extract, batch_size=3)

                if combined_json:
                    logger.info("Successfully combined data")
                    if not os.path.exists(config.Dirs.RESULT_DIR + "/" + str(k) + "_" + pdf_file.filename):
                            os.makedirs(config.Dirs.RESULT_DIR + "/" + str(k) + "_" + pdf_file.filename, exist_ok=True)
                    if extractDataAndSave.save_final_data(combined_json, config.Dirs.RESULT_DIR + "/" + str(k) + "_" + pdf_file.filename + "/" + "combined.json"):
                        logger.info(f"\nProcess completed successfully!")
                        logger.info(f"Individual JSON files saved in: {config.Dirs.JSON_DIR + '/' + str(k) + '_' + pdf_file.filename}/")
